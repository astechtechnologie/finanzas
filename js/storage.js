// storage.js – acceso a Firestore con operaciones CRUD completas
(function() {
  ...
  // Presupuestos
  App.obtenerLimitesCategorias = (mes, cb) => db.collection('usuarios').doc(uid()).collection('presupuestos').doc(mes).get().then(doc => cb(doc.exists ? (doc.data().categorias || {}) : {}));
  App.guardarLimiteCategoria = (mes, cat, limite, cb) => {
    const ref = db.collection('usuarios').doc(uid()).collection('presupuestos').doc(mes);
    db.runTransaction(t => t.get(ref).then(doc => {
      const cats = (doc.exists ? doc.data().categorias : {}) || {};
      cats[cat] = limite;
      return t.set(ref, { categorias: cats }, { merge: true });
    })).then(cb);
  };
  App.obtenerHistorialPresupuestos = (cb) => {
    const meses = [];
    const hoy = new Date();
    for (let i = 2; i >= 0; i--) {
    ...
    Promise.all(meses.map(m => db.collection('usuarios').doc(uid()).collection('presupuestos').doc(m).get())).then(docs => {
      cb(docs.map((doc,i) => ({
        mes: meses[i],
        limiteTotal: doc.exists ? Object.values(doc.data().categorias||{}).reduce((a,b)=>a+b,0) : 0
      })));
    });
  };
})();
